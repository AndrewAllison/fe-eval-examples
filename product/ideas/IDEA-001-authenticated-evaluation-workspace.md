...
A contributor can use their organisation Google Workspace identity or a disposable evaluation account to enter a protected workspace and inspect the requirement coverage of a run.

## Constraints

- Authentication state must be persisted in PostgreSQL.
- Google authentication must reject identities outside the configured Workspace organisation.
- Credential identities must support deterministic automated evaluation.
